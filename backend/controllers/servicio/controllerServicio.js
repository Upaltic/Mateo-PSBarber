...
async function listarServicio(req, res) {
  try {
      ...
      const servicioConCitas = await Promise.all(servicio.map(async (servicio) => {
          const citasAsociadas = await DetalleServicio.findOne({
              where: {
          ...
          return {
              ...servicio.toJSON(),
              tieneCitas: !!citasAsociadas,
          };
      }));
      res.json(servicioConCitas);
